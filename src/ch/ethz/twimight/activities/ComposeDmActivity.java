...
public class ComposeDmActivity extends ThemeSelectorActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.compose_dm);
		...
		textWatcher = new TextWatcher() {
			public void afterTextChanged(Editable s) {
				...
				if (nrCharacters <= 0) {
					...
					characters.setTextColor(getResources().getColor(
							R.color.medium_gray));
				}
		...
		recepient = (EditText) findViewById(R.id.dmRecipient);
		// Did we get some extras in the intent?
		if (i.hasExtra("recipient")) {
			recepient.setText(Html.fromHtml("<i>"
					+ i.getStringExtra("recipient") + "</i>"));
	...
	/**
	 * Checks whether we are in disaster mode and inserts the direct message in
	 * the db *
	 * 
	 * @author pcarta
	 * 
		...
		protected Boolean doInBackground(Void... params) {
			...
			if (cv != null) {
				if (PreferenceManager.getDefaultSharedPreferences(
						ComposeDmActivity.this).getBoolean("prefDisasterMode",
						false) == true) {

					// our own DMs go into the my disaster dm buffer
					cv.put(DirectMessages.COL_BUFFER,
					...
					getContentResolver().insert(
							Uri.parse("content://"
									...
									+ DirectMessages.DMS + "/"
									+ DirectMessages.DMS_LIST + "/"
									+ DirectMessages.DMS_SOURCE_DISASTER), cv);
				} else {
					...
					getContentResolver().insert(
							Uri.parse("content://"
									...
									+ DirectMessages.DMS + "/"
									+ DirectMessages.DMS_LIST + "/"
									+ DirectMessages.DMS_SOURCE_NORMAL), cv);
	...
	private ContentValues createContentValues() {
		...
		try {
			...
			if (text != null && text.getText() != null)
				dmContentValues.put(DirectMessages.COL_TEXT, text.getText()
						.toString());
			dmContentValues.put(DirectMessages.COL_SENDER,
					LoginActivity.getTwitterId(this));
			dmContentValues.put(DirectMessages.COL_RECEIVER_SCREENNAME,
					recepient.getText().toString());
			// we mark the tweet for sending to twitter
			dmContentValues.put(DirectMessages.COL_FLAGS,
					DirectMessages.FLAG_TO_INSERT);
