		...
		if (currentNetworkInfo == null || !currentNetworkInfo.isConnected()) {
			...
			if (twitter == null) {
				...
				if (!(token == null || secret == null)) {
					// get ready for OAuth
					OAuthSignpostClient client = new OAuthSignpostClient(Obfuscator.getKey(), Obfuscator.getSecret(),
							token, secret);
		...
		try {
			...
			for (TweetEntity curEntity : allEntities) {
				...
				if (curEntity.type == KEntityType.hashtags) {
					...
					replacedText.append("<url target='" + originalText.substring(curEntity.start, curEntity.end) + "'>"
							+ curEntity.displayVersion() + "</url>");
					urls.add(curEntity.displayVersion());

				} else if (curEntity.type == KEntityType.user_mentions) {
					replacedText.append("<mention target='" + originalText.substring(curEntity.start, curEntity.end)
				...
				if (ex instanceof TwitterException.RateLimit) {
					...
					if (TweetListActivity.running)
						Toast.makeText(getBaseContext(),
								"Something went wrong when loading your favorites. Please try again later!",
		...
		protected void onPostExecute(List<winterwell.jtwitter.Status> result) {
			...
			if (ex != null) {
				if (ex instanceof TwitterException.RateLimit) {
					...
					if (TweetListActivity.running)
						Toast.makeText(getBaseContext(),
								"Something went wrong when loading your timeline. Please try again later!",
								Toast.LENGTH_SHORT).show();
					Log.e(TAG, "exception while loading timeline: " + ex);
					if (attempts_left > 0)
						new UpdateTimelineTask().execute(--attempts_left);
		...
		protected void onPostExecute(List<winterwell.jtwitter.Status> result) {
			...
			if (ex != null) {
				if (ex instanceof TwitterException.RateLimit) {
					...
					if (TweetListActivity.running)
						Toast.makeText(getBaseContext(),
								"Something went wrong while loading the timeline. Please try again later!",
								Toast.LENGTH_SHORT).show();
					Log.e(TAG, "exception while updating user: " + ex);
				}
				return;
		...
		protected void onPostExecute(List<winterwell.jtwitter.Status> result) {
			...
			if (ex != null) {
				if (ex instanceof TwitterException.RateLimit) {
					...
					if (TweetListActivity.running)
						Toast.makeText(getBaseContext(),
								"Something went wrong while searching. Please try again later!", Toast.LENGTH_SHORT)
								.show();
		...
		protected void onPostExecute(List<Number> result) {
			...
			if (ex != null) {
				if (ex instanceof TwitterException.RateLimit) {
					...
					if (TweetListActivity.running && notify == TRUE)
						Toast.makeText(getBaseContext(),
								"Something went wrong when loading your friends. Please try again later!",
								Toast.LENGTH_SHORT).show();
		...
		protected void onPostExecute(List<Number> result) {
			...
			if (ex != null) {
				if (ex instanceof TwitterException.RateLimit) {
					...
					if (TweetListActivity.running && notify == TRUE)
						Toast.makeText(getBaseContext(),
								"Something went wrong when loading your followers. Please try again later!",
								Toast.LENGTH_SHORT).show();
					Log.e(TAG, "exception while loading followers: " + ex);
				}
				return;
	...
	private class UpdateStatusTask extends AsyncTask<Long, Void, winterwell.jtwitter.Status> {
		...
		protected winterwell.jtwitter.Status doInBackground(Long... rowId) {
			...
			try {
				...
				if (!(c.getDouble(c.getColumnIndex(Tweets.COL_LAT)) == 0 && c.getDouble(c
						.getColumnIndex(Tweets.COL_LNG)) == 0)) {
					double[] location = { c.getDouble(c.getColumnIndex(Tweets.COL_LAT)),
		...
		protected void onPostExecute(winterwell.jtwitter.Status result) {
			...
			if (ex != null) {
				if (ex instanceof TwitterException.Repetition) {
					...
					// we get unexplained exceptions if what twitter returns
					// does not match what we have sent.
					// this does not have to be an error, it happens if we post
					// a url, for example.
					...
					if (TweetListActivity.running && notify == TRUE)
						Toast.makeText(getBaseContext(),
								"Something went wrong while posting your tweet. Please try again later!",
								Toast.LENGTH_SHORT).show();
			...
			if (ex != null) {
				if (ex instanceof TwitterException.Repetition) {
					...
					if (attempts > 0) {
						...
						if (TweetListActivity.running && notify == TRUE)
							Toast.makeText(getBaseContext(),
									"Something went wrong while deleting your tweet. We will try again later!",
									Toast.LENGTH_SHORT).show();
						return;
					}
				} else if (ex instanceof TwitterException.Timeout) {
					...
					if (attempts > 0) {
						...
						if (TweetListActivity.running && notify == TRUE)
							Toast.makeText(getBaseContext(), "Timeout while deleting tweet. We will try again later!",
									Toast.LENGTH_SHORT).show();
						return;
					}
					...
					if (TweetListActivity.running && notify == TRUE)
						Toast.makeText(getBaseContext(),
								"Something went wrong while deleting. We will try again later!", Toast.LENGTH_LONG)
								.show();
			...
			if (ex != null) {
				...
				if (ex instanceof TwitterException.Repetition) {
					...
					if (attempts > 0) {
						...
						if (TweetListActivity.running && notify == TRUE)
							Toast.makeText(getBaseContext(),
									"Something went wrong while deleting your message. We will try again later!",
									Toast.LENGTH_SHORT).show();
						return;
					}
				} else if (ex instanceof TwitterException.Timeout) {
					...
					if (attempts > 0) {
						...
						if (TweetListActivity.running && notify == TRUE)
							Toast.makeText(getBaseContext(),
									"Timeout while deleting message. We will try again later!", Toast.LENGTH_SHORT)
									.show();
						return;
					}
				} else if (ex instanceof TwitterException.E404) {
	...
	private class FavoriteStatusTask extends AsyncTask<Long, Void, Integer> {
		...
		protected Integer doInBackground(Long... rowId) {
			...
			try {
				...
				twitter.setFavorite(
						new winterwell.jtwitter.Status(null, null, c.getLong(c.getColumnIndex(Tweets.COL_TID)), null),
						true);
				result = 1;
			} catch (Exception ex) {
				this.ex = ex;
			...
			if (ex != null) {
				if (ex instanceof TwitterException.Repetition) {
					...
					if (attempts > 0) {
						...
						if (TweetListActivity.running && notify == TRUE)
							Toast.makeText(getBaseContext(), "Something went wrong while favoriting.",
									Toast.LENGTH_SHORT).show();
						return;
					}
				} else if (ex instanceof TwitterException.Timeout) {
					...
					if (TweetListActivity.running && notify == TRUE)
						Toast.makeText(getBaseContext(),
								"Something went wrong while favoriting. We will try again later!", Toast.LENGTH_LONG)
								.show();
					Log.e(TAG, "exception while favoriting: " + ex);
					return;
				}
	...
	private class UnfavoriteStatusTask extends AsyncTask<Long, Void, Integer> {
		...
		protected Integer doInBackground(Long... rowId) {
			...
			if (c != null) {
				...
				try {
					twitter.setFavorite(
							new winterwell.jtwitter.Status(null, null, c.getLong(c.getColumnIndex(Tweets.COL_TID)),
									null), false);
					result = 1;
				} catch (Exception ex) {
				}
			...
			if (ex != null) {
				if (ex instanceof TwitterException.Repetition) {
					...
					if (attempts > 0) {
						...
						if (TweetListActivity.running && notify == TRUE)
							Toast.makeText(getBaseContext(), "Something went wrong while unfavoriting.",
									Toast.LENGTH_SHORT).show();
						return;
					}
				} else if (ex instanceof TwitterException.Timeout) {
					...
					if (TweetListActivity.running && notify == TRUE)
						Toast.makeText(getBaseContext(),
								"Something went wrong while unfavoriting. We will try again later!", Toast.LENGTH_LONG)
								.show();
					if (D)
						Log.e(TAG, "exception while favoriting: " + ex);
					return;
	...
	private class RetweetStatusTask extends AsyncTask<Long, Void, Integer> {
		...
		protected Integer doInBackground(Long... rowId) {
			...
			if (c != null) {
				...
				try {
					twitter.retweet(new winterwell.jtwitter.Status(null, null, c.getLong(c
							.getColumnIndex(Tweets.COL_TID)), null));
			...
			if (ex != null) {
				if (ex instanceof TwitterException.Repetition) {
					...
					if (attempts > 0) {
						...
						if (TweetListActivity.running && notify == TRUE)
							Toast.makeText(getBaseContext(), "Something went wrong retweeting.", Toast.LENGTH_SHORT)
									.show();
						return;
					}
					...
					if (TweetListActivity.running && notify == TRUE)
						Toast.makeText(getBaseContext(),
								"Something went wrong while retweeting. We will try again later!", Toast.LENGTH_LONG)
								.show();
					if (D)
			...
			if (ex != null) {
				if (ex instanceof TwitterException.E401) {
					...
					if (attempts > 0) {
						...
						if (TweetListActivity.running)
							Toast.makeText(getBaseContext(), "Something went wrong while sending follow request.",
									Toast.LENGTH_SHORT).show();
						return;
					}
				} else if (ex instanceof TwitterException.Timeout) {
					...
					if (attempts > 0) {
						...
						if (TweetListActivity.running)
							Toast.makeText(getBaseContext(), "Timeout while sending follow request.",
									Toast.LENGTH_SHORT).show();
						return;
					}
				} else {
					...
					if (TweetListActivity.running)
						Toast.makeText(getBaseContext(),
								"Something went wrong while sending follow request. We will try again later!",
			...
			if (ex != null) {
				if (ex instanceof TwitterException.E401) {
					...
					if (attempts > 0) {
						...
						if (TweetListActivity.running)
							Toast.makeText(getBaseContext(), "Something went wrong while sending unfollow request.",
									Toast.LENGTH_SHORT).show();
						return;
					}
				} else if (ex instanceof TwitterException.Timeout) {
					...
					if (attempts > 0) {
						...
						if (TweetListActivity.running)
							Toast.makeText(getBaseContext(), "Timeout while sending unfollow request.",
									Toast.LENGTH_SHORT).show();
						return;
					}
				} else {
					if (TweetListActivity.running)
						Toast.makeText(getBaseContext(),
								"Something went wrong while sending the unfollow request. We will try again later!",
								Toast.LENGTH_LONG).show();
					if (D)
			...
			if (!result.isEmpty()) {
				...
				for (winterwell.jtwitter.Message dm : result) {
					if (lastId == null) {
						...
						// save the id of the last DM (comes first from twitter)
						// for future synchs
