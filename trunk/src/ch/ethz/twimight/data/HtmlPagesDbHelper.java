...
public class HtmlPagesDbHelper {
	...
	public void saveLinksFromCursor(Cursor c, int type) {
		...
		if (c != null) {
			...
			for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
				...
				insertLinksIntoDb(urlEntities, disId, type);
			}
		}
	}

	public void insertLinksIntoDb(URLEntity[] urlEntities, long tweetId, int type) {

		for (URLEntity urlEntity : urlEntities) {
			insertPage(urlEntity.getExpandedURL(), tweetId, type);
	...
	/**
	 * get the filename of xml file given url and tweet_id, if not found, return
	 * null
	 * 
	 * @param url
	 * @param tweetId
	...
	public Cursor getPageInfo(String url) {
		...
		Cursor c = database.query(DBOpenHelper.TABLE_HTML, null, HtmlPage.COL_URL + " = '" + url + "'", null, null,
				null, null);
	...
	public Cursor getTweetUrls(long tweetId) {
		...
		Cursor c = database.query(DBOpenHelper.TABLE_HTML, null, HtmlPage.COL_DISASTERID + " = " + tweetId, null, null,
				null, null);
	...
	public boolean allPagesStored(Cursor curHtml) {
		...
		if (curHtml.getCount() == 0)
			throw new IllegalArgumentException("The cursor has no elements");
	...
	public Cursor getUndownloadedHtmls(boolean forced) {
		String sql = null;
		if (forced) {
			sql = HtmlPage.COL_FILENAME + " is null and " + HtmlPage.COL_FORCED + " = " + 1 + " and "
					+ HtmlPage.COL_ATTEMPTS + " < " + HtmlPage.DOWNLOAD_LIMIT + "";
		} else {
			sql = HtmlPage.COL_FILENAME + " is null and " + HtmlPage.COL_ATTEMPTS + " < " + HtmlPage.DOWNLOAD_LIMIT
					+ "";
		}
