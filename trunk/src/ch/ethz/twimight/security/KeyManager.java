...
public class KeyManager {
	...
	public KeyPair getKey() {
		...
		// if we had a key saved, this should be true. otherwise we will now
		// create one.
		if (PUBLICModulusString != null && PUBLICExponentString != null && PRIVATEModulusString != null
				&& PRIVATEExponentString != null) {

			try {

				KeyFactory fact = KeyFactory.getInstance("RSA");
				RSAPublicKeySpec pub = new RSAPublicKeySpec(new BigInteger(PUBLICModulusString), new BigInteger(
						PUBLICExponentString));
				RSAPublicKey publicKey = (RSAPublicKey) fact.generatePublic(pub);

				RSAPrivateKeySpec priv = new RSAPrivateKeySpec(new BigInteger(PRIVATEModulusString), new BigInteger(
						PRIVATEExponentString));
	...
	/**
	 * Creates PEM Format of the encoded (PUBLICCS #8?) public key TODO: Let
	 * BouncyCastle (SpongyCastle) take care of this
	 * 
	 * @param kp
	 * @return
	 */
	...
	public static String getPemPublicKey(PublicKey key) {
		...
		while (i < encoded.length()) {
			builder.append(encoded.substring(i, Math.min(i + 64, encoded.length())));
			builder.append("\n");
			i += 64;
		}
	...
	public String getSignature(String text) {
		...
		if (hash != null) {
			try {
				...
				// Log.i(TAG, "my public key: " +
				// getPemPublicKey(kp.getPublic()));
	...
	/**
	 * Checks if a given signature matches the text, for the public key provided
