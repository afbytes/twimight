...
/**
 * TDSAlarm, together with TDSThread form the controller for the communication
 * with the Twimight Disaster Server. TDSAlarm periodically schedules (and
 * receives) Alarms to launch a TDSThread.
 * 
 * @author thossmann
 * 
 */
public class TDSAlarm extends BroadcastReceiver {
	...
	/**
	 * Call this constructor to trigger set everything up and schedule the first
	 * alarm.
	 * 
	 * @param context
	 * @param timeOut
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (isTdsEnabled(context)) {
			...
			// do we have a MAC address now? if not, we have to ask the user to
			// switch on bluetooth, since we cannot obtain the address from the
			// BluetoothAdapter when Bluetooth is off
			else {

				enableBluetooth(context); // this will also schedule a
											// TDSThread, once bluetooth is
											// done.
	...
	/**
	 * Enable Bluetooth for 2 seconds to read the local mac address Note: this
	 * is an ugly hack, please close both eyes here!
	 */
	protected static void enableBluetooth(final Context context) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				int attempts = 0;
				while (PreferenceManager.getDefaultSharedPreferences(context).getString("mac", null) == null
						&& attempts <= 3) {
				...
				if (PreferenceManager.getDefaultSharedPreferences(context).getString("mac", null) != null
						&& PreferenceManager.getDefaultSharedPreferences(context).getBoolean("prefTDSCommunication",
	...
	public static boolean isTdsEnabled(Context context) {
		return PreferenceManager.getDefaultSharedPreferences(context).getBoolean("prefTDSCommunication",
