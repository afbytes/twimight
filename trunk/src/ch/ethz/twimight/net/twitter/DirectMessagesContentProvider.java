...
/**
 * The content provider for all kinds of direct messages (normal, disaster,
 * local and relayed). The URIs and column names are defined in class
 * DirectMessages.
 * 
 * @author thossmann
 * 
...
public class DirectMessagesContentProvider extends ContentProvider {
	...
	static {
		...
		dmUriMatcher.addURI(DirectMessages.DM_AUTHORITY, DirectMessages.DMS + "/" + DirectMessages.DMS_USER + "/#",
				USER);
	...
	public synchronized Cursor query(Uri uri, String[] projection, String where, String[] whereArgs, String sortOrder) {
		...
		case DM_ID:
			...
			sql = "SELECT  * " + "FROM " + DBOpenHelper.TABLE_DMS + " " + "WHERE " + DBOpenHelper.TABLE_DMS + "._id="
		...
		case USER:
			...
			sql = "SELECT " + DBOpenHelper.TABLE_USERS + "." + TwitterUsers.COL_TWITTERUSER_ID + " " + "FROM "
			...
			sql = "SELECT " + DBOpenHelper.TABLE_DMS + "._id, " + DBOpenHelper.TABLE_DMS + "."
					...
					+ DBOpenHelper.TABLE_DMS + "." + DirectMessages.COL_RECEIVER + "=" + userId + " " + "OR "
					+ DBOpenHelper.TABLE_DMS + "." + DirectMessages.COL_SENDER + "=" + userId + " " + "ORDER BY "
		...
		case USERS:
			...
			// recepient
			String subQuery1 = "SELECT " + DirectMessages.COL_TEXT + " " + "FROM " + DBOpenHelper.TABLE_DMS + " "
					+ "WHERE " + DBOpenHelper.TABLE_DMS + "." + DirectMessages.COL_RECEIVER + "="
					+ DBOpenHelper.TABLE_USERS + "." + TwitterUsers.COL_TWITTERUSER_ID + " " + "OR "
					+ DBOpenHelper.TABLE_DMS + "." + DirectMessages.COL_SENDER + "=" + DBOpenHelper.TABLE_USERS + "."
					+ TwitterUsers.COL_TWITTERUSER_ID + " " + "ORDER BY " + DBOpenHelper.TABLE_DMS + "."
			...
			String subQuery2 = "SELECT " + DirectMessages.COL_CREATED + " " + "FROM " + DBOpenHelper.TABLE_DMS + " "
					+ "WHERE " + DBOpenHelper.TABLE_DMS + "." + DirectMessages.COL_RECEIVER + "="
					+ DBOpenHelper.TABLE_USERS + "." + TwitterUsers.COL_TWITTERUSER_ID + " " + "OR "
					+ DBOpenHelper.TABLE_DMS + "." + DirectMessages.COL_SENDER + "=" + DBOpenHelper.TABLE_USERS + "."
					+ TwitterUsers.COL_TWITTERUSER_ID + " " + "ORDER BY " + DBOpenHelper.TABLE_DMS + "."
			...
			sql = "SELECT " + DBOpenHelper.TABLE_USERS + "._id, " + DBOpenHelper.TABLE_USERS + "."
					...
					+ DBOpenHelper.TABLE_USERS + "." + TwitterUsers.COL_TWITTERUSER_ID + "=sent."
					+ DirectMessages.COL_SENDER + " " + "LEFT JOIN " + DBOpenHelper.TABLE_DMS + " AS received ON "
					+ DBOpenHelper.TABLE_USERS + "." + TwitterUsers.COL_TWITTERUSER_ID + "=received."
					...
					+ TwitterUsers.COL_TWITTERUSER_ID + " " + "ORDER BY created DESC LIMIT 100;";
		...
		case LIST_DISASTER:
			...
			c = database.query(DBOpenHelper.TABLE_DMS, projection, DirectMessages.COL_BUFFER + "&"
					+ DirectMessages.BUFFER_MYDISASTER + "!=0" + " OR " + DirectMessages.COL_BUFFER + "&"
					+ DirectMessages.BUFFER_DISASTER_OTHERS + "!=0", whereArgs, null, null, sortOrder);
			c.setNotificationUri(getContext().getContentResolver(), DirectMessages.CONTENT_URI);
			break;
	...
	public synchronized Uri insert(Uri uri, ContentValues values) {
		...
		case LIST_NORMAL:
			...
			c = database.query(DBOpenHelper.TABLE_DMS, null, DirectMessages.COL_DISASTERID + "=" + disasterId, null,
					null, null, null);
			if (c.getCount() > 0) {

				c.moveToFirst();
				while (!c.isAfterLast()) {
					boolean isOurs = Long.toString(c.getLong(c.getColumnIndex(DirectMessages.COL_SENDER))).equals(
							LoginActivity.getTwitterId(getContext()));
					boolean isForUs = Long.toString(c.getLong(c.getColumnIndex(DirectMessages.COL_RECEIVER))).equals(
							LoginActivity.getTwitterId(getContext()));
					...
					if (isOurs || isForUs) {
						...
						Uri updateUri = Uri.parse("content://" + DirectMessages.DM_AUTHORITY + "/" + DirectMessages.DMS
		...
		case LIST_DISASTER:
			...
			disasterId = getDisasterID(values);
			c = database.query(DBOpenHelper.TABLE_DMS, null, DirectMessages.COL_DISASTERID + "=" + disasterId + " AND "
					+ DirectMessages.COL_ISDISASTER + ">0", null, null, null, null);
			if (c.getCount() > 0) {
				c.moveToFirst();
				Uri oldUri = Uri.parse("content://" + DirectMessages.DM_AUTHORITY + "/" + DirectMessages.DMS + "/"
			...
			if (LoginActivity.getTwitterId(getContext()).equals(
					values.getAsInteger(DirectMessages.COL_SENDER).toString())) {
				...
				// Is it for me?
				if (values.getAsLong(DirectMessages.COL_RECEIVER).toString()
						.equals(LoginActivity.getTwitterId(getContext()))) {

					values.put(DirectMessages.COL_BUFFER, DirectMessages.BUFFER_DISASTER_ME
							| DirectMessages.BUFFER_MESSAGES);
	...
	private Long getIdFromScreenName(String screenName) {
		Cursor c = database.query(DBOpenHelper.TABLE_USERS, null,
				TwitterUsers.COL_SCREENNAME + "='" + screenName + "'", null, null, null, null);
		if (c.getCount() > 0) {
			c.moveToFirst();
	...
	private void purgeBuffer(int buffer, int size) {
		/*
		 * First, we remove the respective flag Second, we delete all direct
		 * messages which have no more buffer flags
		 */
		...
		sql = "UPDATE " + DBOpenHelper.TABLE_DMS + " " + "SET " + DirectMessages.COL_BUFFER + "=(" + (~buffer) + "&"
				+ DirectMessages.COL_BUFFER + ") " + "WHERE " + "_id=(SELECT _id FROM " + DBOpenHelper.TABLE_DMS
				+ " WHERE " + sqlWhere + " ORDER BY " + Tweets.DEFAULT_SORT_ORDER + " " + " LIMIT -1 OFFSET " + size
				+ ");";
		c = database.rawQuery(sql, null);
		c.close();
	 ...
	 * the message TODO: For security reasons (to prevent intentional hash
	...
	private Uri insertDM(ContentValues values) {
		if (checkValues(values)) {
			...
			// for our own new messages, we have a receiver screenname but no
			// ID.
			// Check if we have a corresponding ID in the TwitterUsers
			if (!values.containsKey(DirectMessages.COL_RECEIVER)
					&& values.containsKey(DirectMessages.COL_RECEIVER_SCREENNAME)) {
				String[] projection = { TwitterUsers.COL_TWITTERUSER_ID };
				String where = TwitterUsers.COL_SCREENNAME + "='"
						+ values.getAsString(DirectMessages.COL_RECEIVER_SCREENNAME) + "'";
				Cursor c = getContext().getContentResolver()
						.query(Uri.parse("content://" + TwitterUsers.TWITTERUSERS_AUTHORITY + "/"
								+ TwitterUsers.TWITTERUSERS), projection, where, null, null);
				if (c.getCount() > 0) {
					c.moveToFirst();
					values.put(DirectMessages.COL_RECEIVER,
							c.getLong(c.getColumnIndex(TwitterUsers.COL_TWITTERUSER_ID)));
				}
				c.close();
			}
